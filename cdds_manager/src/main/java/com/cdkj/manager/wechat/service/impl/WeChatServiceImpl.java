...
import com.cdkj.manager.wechat.util.MsgProcessUtil;
import com.cdkj.manager.wechat.util.WeChatUtil;
...
import com.cdkj.manager.wechat.util.msg.InMsgParser;
...
//        </xml>
        ...
        if (StringUtils.isEmpty(ticket)) {
            ...
            redisClient.set(RedisKeys.WECHAT_TICKET + appId, ticket, 600);
            logger.debug("redis ticket:" + redisClient.get(RedisKeys.WECHAT_TICKET + appId));
        }
    }
...
//        拿到票据换取公众号授权信息
        ...
        recordAuthInfo(authorizationCode, result, authorizerInfo);
        logger.debug("api query authorizerInfo :{}", result);
     ...
     */
    private void recordAuthInfo(String authorizationCode, String result, String authorizerInfo) {
        ...
        //存储用户access_token
        redisClient.set(RedisKeys.WECHAT_AUTH_ACCESS_TOKEN + authorizerAppid, authorizerAccessToken, 7100);
        //权限列表
        List<Map<String, Object>> funcInfo = (List<Map<String, Object>>) resultMap.get("func_info");
        logger.debug("func_info:", funcInfo);
        //新增或更新授权信息
        wechatFuncInfoService.addOrUpdateSrvFuncInfo(authorizerAccessToken, authorizerRefreshToken, authorizerAppid, authorizationCode, funcInfo, authInfoMap);
...
//        </xml>
    ...
    public String getWeChatApiComponentToken() {
        ...
        String wechatApiComponentToken = redisClient.get(RedisKeys.WECHAT_API_COMPONENT_TOKEN + componentAppId);

        if (StringUtils.isEmpty(wechatApiComponentToken)) {
            //获取新的TOKEN
            wechatApiComponentToken = WeChatUtil.getWeChatApiComponentToken(componentAppId, appSecret, redisClient.get(RedisKeys.WECHAT_TICKET + componentAppId));
            //小于官方的7200，便于防止失效
            if (!StringUtils.isEmpty(wechatApiComponentToken)) {
                redisClient.set(RedisKeys.WECHAT_API_COMPONENT_TOKEN + componentAppId, wechatApiComponentToken, 7100);
            }
    ...
    public String getPreAuthCode() {
        ...
        String preAuthCode = redisClient.get(RedisKeys.WECHAT_PRE_AUTH_CODE + componentAppId);
        if (StringUtils.isEmpty(preAuthCode)) {
            String token = redisClient.get(RedisKeys.WECHAT_API_COMPONENT_TOKEN + componentAppId);
            ...
            //小于官方的1200，便于防止时间差失效
            if (!StringUtils.isEmpty(preAuthCode)) {
                redisClient.set(RedisKeys.WECHAT_PRE_AUTH_CODE + componentAppId, preAuthCode, 1150);
            }
        }
        logger.debug("getPreAuthCode..........................start");
        return preAuthCode;
    ...
    public String getUrlWeChatAuth() {
        ...
        String result = getPreAuthCode();
        return WeChatUtil.getUrlWeChatAuth(componentAppId,result , redirectUri, "1");
    }
